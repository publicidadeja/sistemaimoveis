...
class HookServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // Eventos para sincronização com o ZAP Imóveis
        Event::listen('eloquent.saved: ' . Property::class, function (Property $property) {
            $this->syncWithZapImoveis($property, 'save');
        });

        Event::listen('eloquent.deleted: ' . Property::class, function (Property $property) {
            $this->syncWithZapImoveis($property, 'delete');
        });
    ...
    protected function syncWithZapImoveis(Property $property, string $action)
    {
        try {
            ...
            // Se o imóvel não está aprovado, remove do ZAP (se existir)
            if ($property->moderation_status !== 'approved' && $property->zap_id) {
                $zapService->removeProperty($property);
                return;
            }
            
            // Sincroniza de acordo com a ação
            if ($action === 'save') {
                // Se já tem ID no ZAP, atualiza; senão, envia novo
                if ($property->zap_id) {
                    $zapService->updateProperty($property);
                } elseif ($property->moderation_status === 'approved') {
                    $zapService->sendProperty($property);
                }
            } elseif ($action === 'delete' && $property->zap_id) {
                $zapService->removeProperty($property);
            }
        } catch (Exception $e) {
            Log::error('Erro ao sincronizar com ZAP Imóveis: ' . $e->getMessage(), [
