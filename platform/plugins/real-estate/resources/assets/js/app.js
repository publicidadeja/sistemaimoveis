...
$(document).ready(() => {
    if (window.noticeMessages) {
        window.noticeMessages.forEach(message => {
            Srapid.showNotice(message.type, message.message, message.type === 'error' ? (window.trans && window.trans.error ? window.trans.error : 'Error!') : (window.trans && window.trans.success ? window.trans.success : 'Success!'));
        });
    }
