class Srapid {
    constructor() {
        ...
        Srapid.initResources();
        Srapid.handleCounterUp();
        Srapid.initMediaIntegrate();
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            Srapid.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        Srapid.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    Srapid.showError(data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Srapid.showError(item);
                        });
                    });
                }
            } else {
                Srapid.showError(data.statusText);
            }
        }
    }
    ...
    static initResources() {
        ...
        Srapid.initDatePicker('.datepicker');
        if (jQuery().mCustomScrollbar) {
            Srapid.callScroll($('.list-item-checkbox'));
        }
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', event => {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = Srapid.getViewPort().height -
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown(event => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Srapid.onDragTool);
            $(window).on('mouseup', Srapid.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Srapid.onDragTool);
        $(window).off('mouseup', Srapid.onReleaseTool);
    }
